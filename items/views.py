...
def item_detail(request, pk):
    try:
        clean_pk = str(pk).rstrip('/')
        item = None
        
        # Try fetching by UUID or PK string
        try:
            item = Item.objects.get(pk=clean_pk)
        except Exception:
            item = Item.objects.filter(id=clean_pk).first()

        if not item:
            return Response({
                'status': 'fail',
                'message': f"Item with ID '{clean_pk}' not found"
        ...
        if request.method == 'GET':
            ...
            return Response({
                'status': 'success',
                'message': f"Item with ID '{clean_pk}' successfully deleted"
            }, status=status.HTTP_200_OK)
